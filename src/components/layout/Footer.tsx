...
export default function Footer({ navItems = [], settings = [] }: FooterProps) {
    ...
    const logoText = settings.find(s => s.settingKey === "footer_logo_text")?.settingValue || settings.find(s => s.settingKey === "header_logo_text")?.settingValue || "Hassan";
    const copyrightText = settings.find(s => s.settingKey === "footer_copyright_text")?.settingValue || `© ${currentYear} ${logoText}. All rights reserved.`;

    if (pathname && (pathname.startsWith("/letmein") || pathname.startsWith("/admin"))) return null;

    return (
        <footer className="relative py-12 border-t border-[#FDDFCC]/10 bg-black">
            {/* Background gradient */}
            <div
                        ...
                        <div className="flex flex-wrap justify-center gap-6 md:justify-end">
                            {navItems.map(item => (
                                <Link
                                    key={item.path}
                                    href={item.path}
                                    className="text-sm text-[#FDDFCC]/60 hover:text-primary-500 transition-colors"
                                >
                                    {item.label}
                                </Link>
                            ))}
